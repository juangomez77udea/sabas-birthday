...
const Boss = ({ isRotating, setIsRotating, ...props }) => {
    ...
    // Estado para controlar la visibilidad de las estrellas
    ...
    // Referencias para el movimiento de las estrellas
    const starRefs = useRef(starPositions.map(() => ({ x: 0, y: 0, speedX: Math.random() * 0.02 - 0.01, speedY: Math.random() * 0.02 - 0.01 })))

    // Alternar la visibilidad de las estrellas cada 3 segundos
    useEffect(() => {
    ...
    useEffect(() => {
        if (animations && animations.length > 0) {
            ...
            if (animationNames.length > 0) {
                const safeAnimations = animationNames.filter((name) => !name.includes("star") && !name.includes("star_parent"))
    ...
    const handleKeyDown = useCallback(
        (e) => {
            if (e.key === "ArrowLeft") {
                ...
                if (animations && animations.length > 0) {
                    const animationNames = Object.keys(actions).filter(
                        (name) => !name.includes("star") && !name.includes("star_parent"),
                    )
                    if (animationNames.length > 0) {
                        setAnimationIndex((prev) => (prev + 1) % animationNames.length)
                    }
                }
            }
        },
        [isRotating, setIsRotating, animations, actions],
    )
    ...
    // Animación de movimiento aleatorio para las estrellas
    useFrame(() => {
        if (showStars) {
            ...
            <group name="Sketchfab_Scene">
                <group name="Sketchfab_model" rotation={[-Math.PI / 2, 0, 0]} scale={0.061}>
                    <group name="Root">
                        ...
                        {/* Estrellas - Renderizar dinámicamente */}
                        {showStars && starPositions.map((star, index) => (
                            nodeExists("star_0") && (
                                <group
                                    key={`star_${index}`}
